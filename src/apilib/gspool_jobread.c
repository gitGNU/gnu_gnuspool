/* gspool_jobread.c -- get attributes of job
...
int	gspool_jobread(const int fd, const unsigned flags, const slotno_t slotno, struct apispq *result)
{
	int	ret;
...
int  gspool_jobfindslot(const int fd, const	unsigned flags, const jobno_t jn, const	netid_t nid, slotno_t *slot)
{
	int		ret;
	struct	api_fd	*fdp = gspool_look_fd(fd);
	struct	api_msg		msg;
...
int  gspool_jobfind(const int fd, const unsigned flags, const jobno_t jn, const netid_t nid, slotno_t *slot, struct apispq *jp)
{
	int		ret;
	struct	api_fd	*fdp = gspool_look_fd(fd);
	struct	api_msg		msg;
