/* gspool_ptrlist.c -- API list printers
...
int  gspool_ptrlist(const int fd, const unsigned flags, int *np, slotno_t **slots)
{
	int		ret;
	unsigned	numptrs;
	...
	numptrs = ntohl(msg.un.r_lister.nitems);
	if  (np)
		*np = (int) numptrs;
	...
	if  (numptrs != 0)  {
		unsigned  nbytes = numptrs * sizeof(slotno_t);
