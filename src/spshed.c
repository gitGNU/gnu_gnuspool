...
extern  void  alloc_roam(const netid_t, const char *);
...
extern  void  set_not_server(struct sp_omsg *, const int);
...
#ifdef  HAVE_FCHOWN
        if  (Daemuid)
                Ignored_error = fchown(Xfershm_lockfd, Daemuid, getgid());
#else
        if  (Daemuid)
                Ignored_error = chown(XLOCK_FILE, Daemuid, getgid());
#endif /* HAVE_CHOWN */
...
void  op_process(const int bytes, struct sp_omsg *req)
{
        ...
        case  SN_SHUTHOST:
                clearhost(req->spr_netid, req->spr_arg1);
        ...
        case  SN_NOTSERVER:
                set_not_server(req, 1);
                break;

        case  SN_NOTNOSERV:
                set_not_server(req, 0);
                break;

        case  SN_REQSYNC:
                if  ((rp = find_sync(req->spr_netid, req->spr_arg1))  &&  sendsync(rp))
                        send_endsync(rp);
                break;
...
unsigned  conn_process(const int bytes, struct sp_nmsg *req)
{
        ...
        case  SON_CONNECT:
                ...
                if  ((rp = rattach(&req->spr_n)))               /* Immediate connect */
                        sendsync(rp);
                break;

        case  SON_DISCONNECT:
                shut_host(req->spr_n.hostid);   /* Send shutdown msg if connected */
                clearhost(req->spr_n.hostid, -1);   /* Just look for connection */
                if  (Netm_pid)
                        kill(Netm_pid, NETSHUTSIG);
...
void  process()
{
        ...
        int     bytes;
        unsigned        selt, put, reft = 0, alt, nett = 0;
