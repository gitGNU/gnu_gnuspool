...
#include "config.h"
static  char    rcsid1[] = "@(#) $Id: xmspuser.c,v 1.3 2013/11/09 11:28:45 toadwarble Exp $";           /* We use these in the about message */
...
static void  cb_saveopts(Widget w)
{
        ...
        disp_str = "(Home)";
        confline_arg = digbuf;
        digbuf[0] = alphsort? '1': '0';
        digbuf[1] = '\0';
        proc_save_opts((const char *) 0, "XMSPUSORT", save_confline_opt);
        XtVaGetValues(uwid, XmNwidth, &wid, XmNvisibleItemCount, &items, NULL);
        sprintf(digbuf, "%d", wid);
        confline_arg = digbuf;
        proc_save_opts((const char *) 0, "XMSPUWIDTH", save_confline_opt);
        sprintf(digbuf, "%d", items);
        proc_save_opts((const char *) 0, "XMSPUITEMS", save_confline_opt);
}
...
static void  wstart(int argc, char **argv)
{
        ...
        if  ((arg = optkeyword("XMSPUSORT")))  {
                alphsort = arg[0] != '0';
                free(arg);
        }
        if  ((arg = optkeyword("XMSPUWIDTH")))  {
                uwidth = atoi(arg);
                free(arg);
        }
        if  ((arg = optkeyword("XMSPUITEMS")))  {
                uitems = atoi(arg);
                free(arg);
        }
