...
char *get_ptrtitle()
{
        ...
        if  (!ptr_format)  {
                ...
                char *npf;
                ptr_format = helpprmpt($P{xmspq ptr default format});
                npf = optkeyword("SPQPTRFLD");
                if  (npf)  {
                        if  (ptr_format)
                                free(ptr_format);
                        ptr_format = npf;
...
static void  make_confline_j(FILE *fp, const char *vname)
{
        fprintf(fp, "%s=%s\n", vname, job_format);
}

static void  make_confline_p(FILE *fp, const char *vname)
{
        fprintf(fp, "%s=%s\n", vname, ptr_format);
}
...
void  cb_saveformats(Widget parent, const int ish)
{
        ...
        if  ((ret = proc_save_opts(dir, "SPQJOBFLD", make_confline_j)) != 0)  {
                doerror(parent, ret);
                return;
        }
        if  ((ret = proc_save_opts(dir, "SPQPTRFLD", make_confline_p)) != 0)
                doerror(parent, ret);
}
